...
public class LinkedPositionalList<E> implements PositionalList<E>, Iterable<E> {
    ...
    public void swap(Position<E> p, Position<E> q) {
        ...
        Node<E> qNode = (Node<E>) q;
        
        Node<E> prevNodeP = pNode.getPrev();
        Node<E> nextNodeP = pNode.getNext();
        
        Node<E> prevNodeQ = qNode.getPrev();
        Node<E> nextNodeQ = qNode.getNext();
        
        prevNodeP.setNext(qNode);
        qNode.setPrev(prevNodeP);
        nextNodeP.setPrev(qNode);
        qNode.setNext(nextNodeP);
        
        prevNodeQ.setNext(pNode);
        pNode.setPrev(prevNodeQ);
        nextNodeQ.setPrev(pNode);
        pNode.setNext(nextNodeQ);
    ...
    private class PositionIterator implements Iterator<Position<E>> {
        ...
        public Position<E> next() throws NoSuchElementException {
            if (cursor == null) {
                throw new NoSuchElementException("no queda nada");
            }
            recent = cursor;
            cursor = after(cursor);
            return recent;
